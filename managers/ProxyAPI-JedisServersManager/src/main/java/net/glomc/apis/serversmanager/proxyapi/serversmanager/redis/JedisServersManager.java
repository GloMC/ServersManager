package net.glomc.apis.serversmanager.proxyapi.serversmanager.redis;
...
public class JedisServersManager extends ServersManager implements Runnable, AutoCloseable {
    ...
    public JedisServersManager(String groupId, UnifiedJedis unifiedJedis, BiConsumer<String, HostAndPort> addServerConsumer, Consumer<String> removeServerConsumer) {
        super(groupId, addServerConsumer, removeServerConsumer);
    ...
    public void run() {
        while (!isClosed.get()) {
            try {
                this.unifiedJedis.subscribe(new PubSubHandler(heartBeats, serversData, serversHostAndPort, this.addServerConsumer(), this.removeServerConsumer()), (groupId() + "-proxies"));
            } catch (Throwable t) {
                // t.printStackTrace();
                try {
                    Thread.sleep(2500);
